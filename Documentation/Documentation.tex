...
At the beginning of each frame the allocator deletes all allocated memory and therefore each frame starts with a blank slate. It is not possible to deallocate specific elements from the frame allocator. Due to it being stack based, it is only possible to roll back the stack to a specified marked point. This deallocates all memory that was allocated after the marker was retrieved.
